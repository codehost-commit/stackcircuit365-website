    ...
    <AppSurface path={`stackcircuit.dev/dashboard/p/${project.id}/incidents`} title={project.name} status={project.status}>
      <div className="p-5">
        ...
        <div className="border border-line">
          <div className="grid grid-cols-[8rem_1fr_7rem_6rem] gap-3 border-b border-line bg-paper px-3.5 py-2 font-mono text-[10px] uppercase tracking-wider text-muted">
            <span>ID</span><span>Kind</span><span>Confidence</span><span>Status</span>
          </div>
          {incidents.map((i) => (
            <Link
              key={i.id}
            ...
            >
              ...
              <span className="truncate text-[13px] text-muted">
                {i.kind}
                {i.suspectCommit ? <span className="font-mono text-[11px] text-muted"> · {i.suspectCommit}</span> : null}
              </span>
              <span className="font-mono text-[12px] text-ink">{Math.round(i.confidence * 100)}%</span>
              <Chip tone={i.status === "recovered" ? "green" : i.needsApproval ? "red" : "muted"}>{i.status}</Chip>
            </Link>
          ))}
        </div>
      </div>
